...
public class RegistryBrowser extends ViewPart {
	...
	private void initializeModel() {
		...
		initializeModelJob = new UIJob(Display.getCurrent(),
	...
	private void makeActions() {
		...
		fGroupByBundlesAction = new GroupByAction(PDERuntimeMessages.get().RegistryBrowser_Bundle, BUNDLES);
		...
		fGroupByServicesAction = new GroupByAction(PDERuntimeMessages.get().RegistryBrowser_Service, SERVICES);
				...
				for (Iterator<Bundle> it = bundles.iterator(); it.hasNext();) {
					...
					if ((problems != null) && (problems.getChildren().length > 0)) {
						dialog = new DiagnosticsDialog(getSite().getShell(),
								PDERuntimeMessages.get().RegistryView_diag_dialog_title, null, problems,
								IStatus.WARNING);
						dialog.open();
					} else {
	...
	public String getTitleSummary() {
		...
		if (tree == null)
			return NLS.bind(PDERuntimeMessages.get().RegistryView_titleSummary, (new String[] { "0", "0", type })); //$NON-NLS-1$ //$NON-NLS-2$
