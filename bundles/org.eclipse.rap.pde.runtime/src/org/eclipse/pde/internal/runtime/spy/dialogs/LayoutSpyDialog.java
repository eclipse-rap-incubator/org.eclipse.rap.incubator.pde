...
public class LayoutSpyDialog {
	...
	public LayoutSpyDialog(Shell parentShell) {
		...
		shell.setText(Messages.get().LayoutSpyDialog_shell_text);
		...
		Composite buttonBar = new Composite(shell, SWT.NONE);
		{
			...
			selectWidgetButton.setText(Messages.get().LayoutSpyDialog_button_select_control);
			goUpButton = new Button(buttonBar, SWT.PUSH);
			goUpButton.setText(Messages.get().LayoutSpyDialog_button_open_parent);
			goDownButton = new Button(buttonBar, SWT.PUSH);
			goDownButton.setText(Messages.get().LayoutSpyDialog_button_open_child);
		...
		ISideEffectFactory sideEffectFactory = createFactory(shell);
	...
	private Control getSelectedChild() {
		return (Control) selectedChild.getValue();
	}
	...
	public void updateOverlay() {
		...
		// region.dispose();
		// region = new Region();
		Control child = (Control) selectedChild.getValue();
		if (child != null) {
	...
	private void selectControl() {
		...
		new ControlSelector((Control control) -> {
			if (control != null) {
				openControl(control);
			}
	...
	// /**
	// * Subtracts the given rectangle from the given region unless the
	// rectangle
	// * is empty.
	// */
	...
	private String getWarningMessage(String string) {
		return NLS.bind(Messages.get().LayoutSpyDialog_warning_prefix, string);
	}
		...
		if (child != null) {
				...
				if (!grid.grabExcessHorizontalSpace) {
					if (isHorizontallyScrollable(child) || isGrowableLayout(child, true)) {
						builder.append(getWarningMessage(
								Messages.get().LayoutSpyDialog_warning_grab_horizontally_scrolling));
					}
				}
				if (!grid.grabExcessVerticalSpace) {
					if (isVerticallyScrollable(child) || isGrowableLayout(child, false)) {
						builder.append(getWarningMessage(
								Messages.get().LayoutSpyDialog_warning_grab_vertical_scrolling));
					}
				}
			...
			if (isHorizontallyScrollable(child)) {
				if (widthHintFromLayoutData == SWT.DEFAULT) {
					builder.append(
							getWarningMessage(Messages.get().LayoutSpyDialog_warning_hint_for_horizontally_scrollable));
				}
			}
			...
			if (isVerticallyScrollable(child)) {
				if (heightHintFromLayoutData == SWT.DEFAULT) {
					builder.append(
							getWarningMessage(Messages.get().LayoutSpyDialog_warning_hint_for_vertically_scrollable));
				}
			}
			...
			if (noOpSize.x != bounds.width || noOpSize.y != bounds.height) {
				builder.append(
						getWarningMessage(NLS.bind(Messages.get().LayoutSpyDialog_warning_unexpected_compute_size,
						new Object[] { printHint(widthHint), printHint(heightHint), printPoint(noOpSize) })));
			}

			if (bounds.height < hWrappedSize.y) {
				builder.append(
						getWarningMessage(Messages.get().LayoutSpyDialog_warning_shorter_than_preferred_size));
			}
	...
	private void printReasonControlIsInvisible(StringBuilder builder, Control control) {
		...
		if (bounds.isEmpty()) {
			builder.append(getWarningMessage(Messages.get().LayoutSpyDialog_warning_zero_size));
			return;
		}
		...
		if (parent != null) {
			...
			if (intersection.isEmpty()) {
				builder.append(getWarningMessage(Messages.get().LayoutSpyDialog_warning_bounds_outside_parent));
				return;
			}

			if (intersection.width < bounds.width || intersection.height < bounds.height) {
				builder.append(getWarningMessage(Messages.get().LayoutSpyDialog_warning_control_partially_clipped));
				return;
			}

			if (overlapsSibling(control)) {
				builder.append(getWarningMessage(Messages.get().LayoutSpyDialog_warning_control_overlaps_siblings));
				return;
			}
		}
	}
			...
			if (layout != null) {
				if (layout instanceof GridLayout) {
					...
					if (hasHorizontallyTruncadeControls && !hasHorizontalGrab) {
						builder.append(getWarningMessage(
								Messages.get().LayoutSpyDialog_warning_not_grabbing_horizontally));
					}

					if (hasVerticallyTruncadeControls && !hasVerticalGrab) {
						builder.append(getWarningMessage(
								Messages.get().LayoutSpyDialog_warning_not_grabbing_vertically));
					}
				} else {
