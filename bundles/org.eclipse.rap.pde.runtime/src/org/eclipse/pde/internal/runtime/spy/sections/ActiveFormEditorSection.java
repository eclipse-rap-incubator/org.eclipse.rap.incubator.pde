...
public class ActiveFormEditorSection implements ISpySection {
	...
	public void build(ScrolledForm form, SpyFormToolkit toolkit, ExecutionEvent event) {
		...
		buffer.append(toolkit.createClassSection(text,
				NLS.bind(PDERuntimeMessages.get().SpyDialog_activePart_desc, "editor tab"), activePage.getClass())); //$NON-NLS-1$

		SectionPart activeSection = getActiveFormSection(activePage);
		if (activeSection != null) {
			buffer.append(toolkit.createClassSection(text,
					NLS.bind(PDERuntimeMessages.get().SpyDialog_activePart_desc, "form section"), //$NON-NLS-1$
