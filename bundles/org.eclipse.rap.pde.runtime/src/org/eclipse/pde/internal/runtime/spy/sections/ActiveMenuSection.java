...
public class ActiveMenuSection implements ISpySection {
	...
	public void build(ScrolledForm form, SpyFormToolkit toolkit, Event event) {
		...
		if (object != null) {
			...
			if (object instanceof IContributionItem) {
				...
				if (id != null) {
					buffer.append(toolkit.createIdentifierSection(text, PDERuntimeMessages.get().ActiveMenuSection_0,
							new String[] { id }));
				}
	...
	private void createLocationURI(SpyFormToolkit toolkit, Object object, FormText text, StringBuilder buffer, String id) {
		...
		if (parent instanceof IMenuManager) {
			...
			buffer.append(toolkit.createIdentifierSection(text, PDERuntimeMessages.get().ActiveMenuSection_7,
					new String[] { locationURI }));
					...
					if (item.getControl() == bar) {
						...
						if (o instanceof ToolBarContributionItem) {
							...
							buffer.append(toolkit.createIdentifierSection(text,
									PDERuntimeMessages.get().ActiveMenuSection_7, new String[] { locationURI }));
						}
						continue;
					}
				}
	...
	private void scan(IContributionItem item, StringBuilder buffer, SpyFormToolkit toolkit, FormText text) {
		...
		if (item instanceof IActionSetContributionItem) {
			...
			buffer.append(toolkit.createIdentifierSection(text, PDERuntimeMessages.get().ActiveMenuSection_1,
		...
		if (item instanceof ActionContributionItem) {
			...
			buffer.append(
					toolkit.createClassSection(text, PDERuntimeMessages.get().ActiveMenuSection_2, command.getClass()));
			buffer.append(toolkit.createClassSection(text, PDERuntimeMessages.get().ActiveMenuSection_3,
	...
	private void createActionContributionItemText(Object object, StringBuilder buffer, SpyFormToolkit toolkit, FormText text) {
		...
		if (id != null) {
			buffer.append(toolkit.createIdentifierSection(text, PDERuntimeMessages.get().ActiveMenuSection_4,
					new String[] { action.getActionDefinitionId() }));
		}
		...
		if (action instanceof PluginAction) {
			...
			buffer.append(toolkit.createClassSection(text, PDERuntimeMessages.get().ActiveMenuSection_5,
					new Class[] { clazz }));
	...
	private void createActionContributionItemText(Object object, StringBuilder buffer, SpyFormToolkit toolkit,
		...
		try {
			...
			buffer.append(toolkit.createClassSection(text, PDERuntimeMessages.get().ActiveMenuSection_6,
